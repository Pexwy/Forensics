...
function Update-UI {
    param($Controls, $StageText, $LogMessage, $ProgressPercent, $StatusText, $StatusColor = 'LightGreen')
    ...
    $Controls.Form.Invoke([Action]{
        ...
        if ($StatusText) {
            ...
            try { $Controls.StatusLabel.ForeColor = [Drawing.Color]::$StatusColor } catch {}
        }
        # FIX: Use [datetime]::Now subtraction to avoid parser ambiguity
        $elapsedSeconds = 0
        if ($Script:ScanStart -ne $null) {
            $elapsedSeconds = [math]::Round(([datetime]::Now - $Script:ScanStart).TotalSeconds, 1)
        }
        $Controls.StatusTime.Text = "  |  Elapsed: ${elapsedSeconds}s"
        $Controls.StatusCounter.Text = "  |  $($Script:Results.Count) artifact categories"
    })
}

function Add-TreeNode {
    param($TreeView, $ParentName, $NodeName, $Color = 'White')
...
function Show-Detail {
    ...
    $sb.AppendLine("Found at: $($result.Timestamp.ToString('yyyy-MM-dd HH:mm:ss'))")
    $sb.AppendLine('')

    $data = $result.Data
    if ($data -is [array] -and $data.Count -gt 0) {
        $sb.AppendLine("Total items: $($data.Count)`n")
        $formatted = $data | Format-Table -AutoSize -Wrap | Out-String -Width 200
        $sb.Append($formatted)
    } elseif ($data -is [string] -and $data.Length -gt 0) {
        $sb.AppendLine($data)
    } else {
        $sb.AppendLine('(No detail data available)')
    }
...
function Start-ForensicScan {
    ...
    Update-UI $Controls -StageText 'Starting scan...' -LogMessage 'Forensic scanner initialized.' -ProgressPercent 0 -StatusText 'Scanning...' -StatusColor 'Yellow'
    ...
    try {
        ...
        $currentStep++
        Update-UI $Controls -StageText "[$currentStep/$totalSteps] Running / Suspicious Processes" -LogMessage 'Enumerating process list...' -ProgressPercent ([int](($currentStep/$totalSteps)*100))
        Get-ProcessArtifacts $Controls

        $currentStep++
        Update-UI $Controls -StageText "[$currentStep/$totalSteps] Prefetch Artifact Analysis" -LogMessage 'Reading Prefetch directory...' -ProgressPercent ([int](($currentStep/$totalSteps)*100))
        Get-PrefetchArtifacts $Controls

        $currentStep++
        Update-UI $Controls -StageText "[$currentStep/$totalSteps] Registry - Installed Software" -LogMessage 'Scanning uninstall keys...' -ProgressPercent ([int](($currentStep/$totalSteps)*100))
        Get-RegistryArtifacts $Controls

        $currentStep++
        Update-UI $Controls -StageText "[$currentStep/$totalSteps] Registry - UserAssist & MRU" -LogMessage 'Checking UserAssist execution history...' -ProgressPercent ([int](($currentStep/$totalSteps)*100))
        Get-UserAssistArtifacts $Controls

        $currentStep++
        Update-UI $Controls -StageText "[$currentStep/$totalSteps] Registry - Shim Database / AppCompat" -LogMessage 'Checking compatibility shims...' -ProgressPercent ([int](($currentStep/$totalSteps)*100))
        Get-AppCompatArtifacts $Controls

        $currentStep++
        Update-UI $Controls -StageText "[$currentStep/$totalSteps] Filesystem - Suspicious Files" -LogMessage 'Scanning directories for cheat files...' -ProgressPercent ([int](($currentStep/$totalSteps)*100))
        Get-FilesystemArtifacts $Controls

        $currentStep++
        Update-UI $Controls -StageText "[$currentStep/$totalSteps] Filesystem - Recent Files / Jumplists" -LogMessage 'Checking recent file traces...' -ProgressPercent ([int](($currentStep/$totalSteps)*100))
        Get-RecentFileArtifacts $Controls

        $currentStep++
        Update-UI $Controls -StageText "[$currentStep/$totalSteps] Event Log - Process Creation (4688)" -LogMessage 'Querying Security event log...' -ProgressPercent ([int](($currentStep/$totalSteps)*100))
        Get-Event4688 $Controls

        $currentStep++
        Update-UI $Controls -StageText "[$currentStep/$totalSteps] Sysmon - Process (ID 1) & DLL (ID 7)" -LogMessage 'Checking Sysmon logs...' -ProgressPercent ([int](($currentStep/$totalSteps)*100))
        Get-SysmonArtifacts $Controls

        $currentStep++
        Update-UI $Controls -StageText "[$currentStep/$totalSteps] Event Log - Anti-Cheat Errors" -LogMessage 'Scanning Application log for anti-cheat events...' -ProgressPercent ([int](($currentStep/$totalSteps)*100))
        Get-AntiCheatErrors $Controls

        $currentStep++
        Update-UI $Controls -StageText "[$currentStep/$totalSteps] Event Log - Service & Driver Loads" -LogMessage 'Checking service/driver installation events...' -ProgressPercent ([int](($currentStep/$totalSteps)*100))
        Get-ServiceDriverEvents $Controls

        $currentStep++
        Update-UI $Controls -StageText "[$currentStep/$totalSteps] Anti-Cheat Data Directories" -LogMessage 'Browsing anti-cheat installation paths...' -ProgressPercent ([int](($currentStep/$totalSteps)*100))
        Get-AntiCheatLogs $Controls

        $currentStep++
        Update-UI $Controls -StageText "[$currentStep/$totalSteps] Network & DNS Artifacts" -LogMessage 'Checking DNS cache and TCP connections...' -ProgressPercent ([int](($currentStep/$totalSteps)*100))
        Get-NetworkArtifacts $Controls

        $currentStep++
        Update-UI $Controls -StageText "[$currentStep/$totalSteps] Game Process Timeline" -LogMessage 'Building execution timelines...' -ProgressPercent ([int](($currentStep/$totalSteps)*100))
        Get-GameTimelines $Controls

        $currentStep++
        Update-UI $Controls -StageText "[$currentStep/$totalSteps] Persistence - Tasks, Services, Autoruns" -LogMessage 'Checking persistence mechanisms...' -ProgressPercent ([int](($currentStep/$totalSteps)*100))
        Get-PersistenceArtifacts $Controls

        $currentStep++
        Update-UI $Controls -StageText "[$currentStep/$totalSteps] WMI & PowerShell History" -LogMessage 'Checking WMI and PS history persistence...' -ProgressPercent ([int](($currentStep/$totalSteps)*100))
        Get-WmiPowerShellArtifacts $Controls

        Update-UI $Controls -StageText 'Scan Complete.' -LogMessage "Scan finished. $($Script:Results.Count) artifact categories collected." -ProgressPercent 100 -StatusText 'Scan Complete' -StatusColor 'LightGreen'

    } catch {
        Update-UI $Controls -LogMessage "ERROR: $($_.Exception.Message)" -StatusText 'Error during scan' -StatusColor 'Red'
    ...
    foreach ($result in $Script:Results) {
        ...
        Add-TreeNode $Controls.TreeView -ParentName $result.Severity -NodeName "${iconPrefix}$($result.Category)" -Color $color
    }
}
...
function Get-ProcessArtifacts {
    param($Controls)
    $suspicious = Get-Process | Where-Object { $_.ProcessName -match ($Script:SusKeywords -join '|') } |
        Select-Object ProcessName, Id, @{N='StartTime';E={$_.StartTime.ToLocalTime()}}, CPU, SessionId
    if ($suspicious) {
        Update-UI $Controls -LogMessage "  → Found $($suspicious.Count) suspicious processes."
        Add-Result -Category 'SuspiciousProcesses' -Severity High -Data $suspicious
    }
    $allProcs = Get-Process | ForEach-Object {
        $p = $_
        $parentId = try { (Get-CimInstance Win32_Process -Filter "ProcessId=$($p.Id)" -ErrorAction Stop).ParentProcessId } catch { $null }
        [PSCustomObject]@{
...
function Get-PrefetchArtifacts {
    ...
    $suspicious = $pfFiles | Where-Object { $_.Name -match ($Script:SusKeywords -join '|') } |
        Select-Object Name, Length, @{N='LastRun';E={$_.LastWriteTime}}
    if ($suspicious) {
        Update-UI $Controls -LogMessage "  → Found $($suspicious.Count) suspicious Prefetch entries."
        Add-Result -Category 'SuspiciousPrefetch' -Severity High -Data $suspicious
    }
    $allPf = $pfFiles | Select-Object Name, Length, @{N='LastRun';E={$_.LastWriteTime}},
        @{N='Executable';E={$_ -replace '-[0-9A-F]{8}\.pf$', '.exe'}} | Sort-Object LastWriteTime -Descending
    Add-Result -Category 'AllPrefetchEntries' -Severity Info -Data $allPf
    Update-UI $Controls -LogMessage "  → $($pfFiles.Count) total Prefetch entries."
}
...
function Get-RegistryArtifacts {
    ...
    if ($suspicious) {
        Update-UI $Controls -LogMessage "  → Found $($suspicious.Count) suspicious installed programs."
...
function Get-UserAssistArtifacts {
    ...
    if ($entries) {
        ...
        Update-UI $Controls -LogMessage "  → $($entries.Count) UserAssist entries captured."
    }
}
...
function Get-AppCompatArtifacts {
    ...
    if ($shimEntries) {
        ...
        Update-UI $Controls -LogMessage "  → $($shimEntries.Count) AppCompat/Layer entries found."
    }
}

function Get-FilesystemArtifacts {
    param($Controls)
    $targetDirs = @("$env:TEMP","$env:APPDATA","$env:LOCALAPPDATA","$env:PROGRAMDATA",
    ...
    if ($found) {
        Update-UI $Controls -LogMessage "  → Found $($found.Count) suspicious files on disk."
    ...
    $dlls = Get-ChildItem "$env:TEMP" -Filter '*.dll' -Recurse -Depth 2 -ErrorAction SilentlyContinue |
        Where-Object { $_.Length -gt 5120 } | Select-Object FullName, Length, LastWriteTime
    if ($dlls) {
        Add-Result -Category 'SuspiciousDLLs_InTemp' -Severity Medium -Data $dlls
        Update-UI $Controls -LogMessage "  → $($dlls.Count) suspicious DLLs in Temp."
    }
    ...
    if ($dumps) {
        ...
        Update-UI $Controls -LogMessage "  → $($dumps.Count) memory dump files found."
    }
}
...
function Get-RecentFileArtifacts {
    ...
    $recentDocs = Get-ChildItem "$env:USERPROFILE\Recent" -ErrorAction SilentlyContinue |
        Where-Object { -not $_.PSIsContainer } | Select-Object Name, Length, LastWriteTime
    if ($recentDocs) {
        Add-Result -Category 'RecentDocuments' -Severity Info -Data $recentDocs
        Update-UI $Controls -LogMessage "  → $($recentDocs.Count) recent document shortcuts."
    }
    ...
    if (Test-Path $jumpListPath) {
        ...
        if ($jumpLists) {
            ...
            Update-UI $Controls -LogMessage "  → $($jumpLists.Count) jumplist files found."
        }
    }
}
...
function Get-Event4688 {
    ...
    try {
        $evts = Get-WinEvent -FilterHashtable @{
            ...
            $xml = [xml]$_.ToXml()
            $data = @{}; $xml.Event.EventData.Data | ForEach-Object { $data[$_.Name] = $_.'#text' }
            [PSCustomObject]@{
                Time=$_.TimeCreated; User=$data['SubjectUserName']
                NewProc=$data['NewProcessName']; PID=$data['NewProcessId']
                CmdLine=$data['CommandLine']; ParentProc=$data['ParentProcessName']
            }
        }
        ...
        if ($suspicious) {
            Update-UI $Controls -LogMessage "  → $($suspicious.Count) suspicious process creations (4688)."
        ...
        Update-UI $Controls -LogMessage "  → $($evts.Count) total process creation events."
    } catch {
        Update-UI $Controls -LogMessage "  → Cannot read Security log: $($_.Exception.Message)"
    }
}
...
function Get-SysmonArtifacts {
    ...
    if (-not $sysmonRunning) {
        Update-UI $Controls -LogMessage '  → Sysmon not installed/running. Skipping.'
        return
    }
    try {
        $sysmon1 = Get-WinEvent -FilterHashtable @{
            LogName='Microsoft-Windows-Sysmon/Operational'; ID=1; StartTime=$Script:SessionStart; EndTime=$Script:SessionEnd
        } -MaxEvents 5000 -ErrorAction Stop | ForEach-Object {
            $xml = [xml]$_.ToXml(); $data = @{}
            $xml.Event.EventData.Data | ForEach-Object { $data[$_.Name] = $_.'#text' }
            [PSCustomObject]@{ Time=$_.TimeCreated; Image=$data['Image']; CmdLine=$data['CommandLine']; PID=$data['ProcessId']; Hashes=$data['Hashes'] }
        }
        ...
        if ($sus1) {
            Update-UI $Controls -LogMessage "  → $($sus1.Count) suspicious Sysmon process events."
        ...
        Update-UI $Controls -LogMessage "  → $($sysmon1.Count) Sysmon process events."
    } catch {}
    try {
        $sysmon7 = Get-WinEvent -FilterHashtable @{
            LogName='Microsoft-Windows-Sysmon/Operational'; ID=7; StartTime=$Script:SessionStart; EndTime=$Script:SessionEnd
        } -MaxEvents 5000 -ErrorAction Stop | ForEach-Object {
            $xml = [xml]$_.ToXml(); $data = @{}
            $xml.Event.EventData.Data | ForEach-Object { $data[$_.Name] = $_.'#text' }
            [PSCustomObject]@{ Time=$_.TimeCreated; Process=$data['Image']; Module=$data['ModuleLoaded']; Signed=$data['Signed'] }
        }
        ...
        if ($susDll) {
            Update-UI $Controls -LogMessage "  → $($susDll.Count) suspicious DLL loads."
        ...
        Update-UI $Controls -LogMessage "  → $($sysmon7.Count) total DLL load events."
    } catch {}
    try {
        $sysmon11 = Get-WinEvent -FilterHashtable @{
            LogName='Microsoft-Windows-Sysmon/Operational'; ID=11; StartTime=$Script:SessionStart; EndTime=$Script:SessionEnd
        } -MaxEvents 3000 -ErrorAction Stop | ForEach-Object {
            $xml = [xml]$_.ToXml(); $data = @{}
            $xml.Event.EventData.Data | ForEach-Object { $data[$_.Name] = $_.'#text' }
            [PSCustomObject]@{ Time=$_.TimeCreated; Image=$data['Image']; Target=$data['TargetFilename'] }
        }
        $susFile = $sysmon11 | Where-Object { $_.Target -match ($Script:SusKeywords -join '|') }
        if ($susFile) {
            Update-UI $Controls -LogMessage "  → $($susFile.Count) suspicious file creations."
            Add-Result -Category 'Sysmon11_SuspiciousFileDrop' -Severity High -Data $susFile
        }
        Add-Result -Category 'Sysmon11_AllFileCreations' -Severity Info -Data $sysmon11
    } catch {}
    try {
        $sysmon22 = Get-WinEvent -FilterHashtable @{
            LogName='Microsoft-Windows-Sysmon/Operational'; ID=22; StartTime=$Script:SessionStart; EndTime=$Script:SessionEnd
        } -MaxEvents 1000 -ErrorAction Stop | ForEach-Object {
            $xml = [xml]$_.ToXml(); $data = @{}
            $xml.Event.EventData.Data | ForEach-Object { $data[$_.Name] = $_.'#text' }
            [PSCustomObject]@{ Time=$_.TimeCreated; Process=$data['Image']; Query=$data['QueryName'] }
        }
        ...
        if ($susDns) {
            ...
            Update-UI $Controls -LogMessage "  → $($susDns.Count) suspicious DNS queries."
        }
...
function Get-AntiCheatErrors {
    ...
    $acKeywords = @(
        ...
        'anticheat', 'valorant', 'fortnite', 'cod', 'warzone',
        'csgo', 'cs2', 'apex', 'r6', 'rainbow', 'siege',
        'destiny', 'overwatch', 'lol', 'league', 'dota',
        'rust', 'escapefromtarkov', 'fivem', 'minecraft'
    )
    try {
        $acEvents = Get-WinEvent -FilterHashtable @{
            LogName='Application'; StartTime=$Script:SessionStart; EndTime=$Script:SessionEnd
        } -MaxEvents 5000 -ErrorAction Stop | Where-Object {
            $msg = $_.Message.ToLower()
            ($acKeywords | Where-Object { $msg -match ([regex]::Escape($_)) }) -or
            $msg -match 'cheat|banned|kicked|blocked|violation|detected|suspicious|unauthorized|memory|injection|hook'
        ...
        if ($acEvents) {
            ...
            Update-UI $Controls -LogMessage "  → Found $($acEvents.Count) anti-cheat events."
        } else {
            Update-UI $Controls -LogMessage '  → No anti-cheat error events found.'
        }
    } catch {
        Update-UI $Controls -LogMessage "  → Error reading Application log: $($_.Exception.Message)"
    }
}
...
function Get-ServiceDriverEvents {
    ...
    try {
        $svcEvents = Get-WinEvent -FilterHashtable @{
            LogName='System'; ID=7034,7035,7045; StartTime=$Script:SessionStart; EndTime=$Script:SessionEnd
        } -MaxEvents 2000 -ErrorAction Stop | ForEach-Object {
            $msg = $_.Message
            [PSCustomObject]@{ Time=$_.TimeCreated; ID=$_.Id; Message=$msg.Substring(0, [math]::Min(200, $msg.Length)) }
        }
        ...
        if ($susSvc) {
            ...
            Update-UI $Controls -LogMessage "  → $($susSvc.Count) service/driver events."
        }
        $driverEvents = Get-WinEvent -FilterHashtable @{
            LogName='System'; ID=6; StartTime=$Script:SessionStart; EndTime=$Script:SessionEnd
        } -MaxEvents 1000 -ErrorAction Stop | ForEach-Object {
            $msg = $_.Message
            [PSCustomObject]@{ Time=$_.TimeCreated; Message=$msg.Substring(0, [math]::Min(200, $msg.Length)) }
        }
        ...
        if ($susDrv) {
            ...
            Update-UI $Controls -LogMessage "  → $($susDrv.Count) suspicious driver loads."
        }
    } catch {
        Update-UI $Controls -LogMessage "  → Error reading System log: $($_.Exception.Message)"
    }
}
...
function Get-AntiCheatLogs {
    param($Controls)
    $acPaths = @(
        "$env:PROGRAMDATA\EasyAntiCheat", "$env:PROGRAMDATA\BattlEye",
        "$env:PROGRAMDATA\FaceIt", "$env:APPDATA\Vanguard",
        "$env:LOCALAPPDATA\Riot Games", "$env:LOCALAPPDATA\FortniteGame",
    ...
    foreach ($acPath in $acPaths) {
        if (Test-Path $acPath) {
            ...
            if ($files) {
                ...
                Update-UI $Controls -LogMessage "  → $dirName : $($files.Count) files found."
            }
        }
    }
}
...
function Get-NetworkArtifacts {
    ...
    if ($dnsSuspicious) {
        ...
        Update-UI $Controls -LogMessage "  → $($dnsSuspicious.Count) suspicious DNS entries."
    }
    $allDns = Get-DnsClientCache -ErrorAction SilentlyContinue
    Add-Result -Category 'AllDNSCache' -Severity Info -Data $allDns
    Update-UI $Controls -LogMessage "  → $($allDns.Count) total DNS cache entries."
    ...
    $highRisk = $tcpConns | Where-Object {
        foreach ($pattern in $Script:HighRiskIPRanges) { if ($_.RemoteAddress -match $pattern) { $true; break } }
        $false
    }
    if ($highRisk) {
        Add-Result -Category 'HighRiskTCPConnections' -Severity High -Data $highRisk
        Update-UI $Controls -LogMessage "  → $($highRisk.Count) connections to high-risk IP ranges."
    }
    Add-Result -Category 'ActiveTCPConnections' -Severity Info -Data $tcpConns
    Update-UI $Controls -LogMessage "  → $($tcpConns.Count) established connections."
    ...
    if ($browserHits) {
        ...
        Update-UI $Controls -LogMessage "  → $($browserHits.Count) browser history databases."
    }
}
...
function Get-GameTimelines {
    ...
    try {
        ...
        foreach ($game in $Script:GameProcessNames) {
            $matches = $evts | Where-Object { $_.Image -match $game -replace '\.exe$','' }
            if ($matches) {
                Add-Result -Category "GameTimeline_$game" -Severity Info -Data ($matches | Sort-Object Time)
                $count++
            }
        }
        if ($count -gt 0) {
            Update-UI $Controls -LogMessage "  → $count games had process events in window."
        } else {
            Update-UI $Controls -LogMessage '  → No game process events found in session window.'
        }
    } catch {
        Update-UI $Controls -LogMessage "  → Error building game timelines: $($_.Exception.Message)"
    }
}
...
function Get-PersistenceArtifacts {
    ...
    $schedTasks = Get-ScheduledTask -ErrorAction SilentlyContinue |
        Where-Object { $_.TaskName -match ($Script:SusKeywords -join '|') -or $_.TaskPath -match ($Script:SusKeywords -join '|') } |
        Select-Object TaskName, TaskPath, State, Author
    if ($schedTasks) {
        Add-Result -Category 'Persistence_ScheduledTasks' -Severity High -Data $schedTasks
        Update-UI $Controls -LogMessage "  → $($schedTasks.Count) suspicious scheduled tasks."
    }
    $allTasks = Get-ScheduledTask -ErrorAction SilentlyContinue | Select-Object TaskName, TaskPath, State, Author
    ...
    if ($svcs) {
        ...
        Update-UI $Controls -LogMessage "  → $($svcs.Count) suspicious services."
    }
    $autoruns = @()
    foreach ($key in $Script:RunKeyPaths) {
        $autoruns += Get-ItemProperty $key -ErrorAction SilentlyContinue | Select-Object * -ExcludeProperty PS*
    }
    if ($autoruns) {
        Add-Result -Category 'Persistence_RegistryRunKeys' -Severity Medium -Data $autoruns
        Update-UI $Controls -LogMessage "  → $($autoruns.Count) registry Run key entries."
    }
    $startupItems = @()
    ...
    if ($startupItems) {
        ...
        Update-UI $Controls -LogMessage "  → $($startupItems.Count) startup folder items."
    }
}
...
function Get-WmiPowerShellArtifacts {
    ...
    try {
        ...
        $wmiSuspects = @()
        if ($wmiBindings) {
            $wmiSuspects += $wmiBindings | Where-Object {
                $_.Query -match ($Script:SusKeywords -join '|') -or $_.Name -match ($Script:SusKeywords -join '|')
            }
        }
        if ($wmiConsumers) {
            $wmiSuspects += $wmiConsumers | Where-Object {
                $_.Name -match ($Script:SusKeywords -join '|') -or
                $_.CommandLineTemplate -match ($Script:SusKeywords -join '|')
            }
        }
        if ($wmiSuspects) {
            Add-Result -Category 'WMI_Persistence' -Severity Critical -Data $wmiSuspects
            Update-UI $Controls -LogMessage "  → $($wmiSuspects.Count) WMI persistence bindings found."
        } else {
            Update-UI $Controls -LogMessage "  → No WMI persistence artifacts."
        }
    } catch {
        Update-UI $Controls -LogMessage "  → WMI check error: $($_.Exception.Message)"
    }
    ...
    if (Test-Path $psHistoryPath) {
        ...
        $susCommands = [System.Collections.Generic.List[string]]::new()
        ...
        if ($susCommands.Count -gt 0) {
            ...
            Update-UI $Controls -LogMessage "  → $($susCommands.Count) suspicious PowerShell commands."
        }
        Add-Result -Category 'PowerShellHistory' -Severity Info -Data $psHistory
        Update-UI $Controls -LogMessage "  → $($psHistory.Count) total PowerShell history entries."
    } else {
        Update-UI $Controls -LogMessage "  → No PowerShell history file found."
    }
}
...
function Export-Report {
    ...
    try {
        $sb = [System.Text.StringBuilder]::new()
        $sb.AppendLine("=" * 80)
        $sb.AppendLine("  GAME CHEAT FORENSIC REPORT")
        $sb.AppendLine("  Generated: $(Get-Date)")
        $sb.AppendLine("  Analyst Tool: HackerAI Forensics Scanner v2.1")
        $sb.AppendLine("=" * 80)
        $sb.AppendLine("")
        $sb.AppendLine("Session Window : $($Script:SessionStart)  →  $($Script:SessionEnd)")
        $sb.AppendLine("Target System  : $env:COMPUTERNAME")
        $sb.AppendLine("OS             : $((Get-CimInstance Win32_OperatingSystem).Caption)")
        $sb.AppendLine("Artifacts Found: $($Script:Results.Count) categories")
        $sb.AppendLine("Errors         : $($Script:ScanErrors.Count)")
        $sb.AppendLine("")

        foreach ($result in $Script:Results) {
            $sb.AppendLine("[$($result.Severity)] $($result.Category)")
            $sb.AppendLine("-" * 60)
            if ($result.Data -is [array] -and $result.Data.Count -gt 0) {
                ...
                $sb.AppendLine("  (data available in XML export)")
            }
            $sb.AppendLine("")
        }
        ...
        if ($Script:ScanErrors.Count -gt 0) {
            ...
            $sb.AppendLine("=" * 60)
            foreach ($err in $Script:ScanErrors) { $sb.AppendLine("  ⚠ $err") }
        }
        ...
        Update-UI $Controls -LogMessage "Report exported to: $($saveDlg.FileName)" -StatusText 'Report Exported' -StatusColor 'LightGreen'
    } catch {
...
$controls.BtnStart.Add_Click({
    ...
    Start-ForensicScan $controls
})

$controls.BtnCancel.Add_Click({
    $Script:CancelScan = $true
    $controls.BtnCancel.Enabled = $false
    Update-UI $controls -StatusText 'Cancelling...' -StatusColor 'Yellow'
})

$controls.BtnExport.Add_Click({ Export-Report $controls })
...
$controls.SessionApply.Add_Click({
    ...
    Update-UI $controls -LogMessage "Session window updated: last $h hours"
})

$controls.TreeView.Add_AfterSelect({ Show-Detail $controls.TreeView $controls.DetailBox $Script:Results })
