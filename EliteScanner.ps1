...
function New-MainForm {
    ...
    $form.Icon          = [System.Drawing.Icon]::ExtractAssociatedIcon(
                              [System.Environment]::SystemDirectory + '\shell32.dll', 41)
    ...
    $subtitle.Text = "Session: $($Script:SessionStart.ToString('yyyy-MM-dd HH:mm')) → $($Script:SessionEnd.ToString('HH:mm'))  |  Duration: $SessionHours hrs  |  $(if ([Security.Principal.WindowsPrincipal]::new([Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {'ADMIN'} else {'USER'})"
...
function Update-UI {
    ...
    $Controls.Form.Invoke([Action]{
        ...
        if ($ProgressPercent -ge 0) { $Controls.ProgressBar.Value = [Math]::Min(100, $ProgressPercent) }
        if ($StatusText)   { 
            $Controls.StatusLabel.Text = " $StatusText"
            $Controls.StatusLabel.ForeColor = [Drawing.Color]::$StatusColor
        }
        $elapsed = [math]::Round((Get-Date - $Script:ScanStart).TotalSeconds, 1)
...
function Start-ForensicScan {
    ...
    foreach ($result in $Script:Results) {
        ...
        Add-TreeNode -TreeView $Controls.TreeView -ParentName $result.Severity -NodeName "${iconPrefix}$($result.Category)" -Color $color
    }
}
...
function Get-NetworkArtifacts {
    ...
    $highRisk = $tcpConns | Where-Object {
        foreach ($pattern in $Script:HighRiskIPRanges) {
            if ($_.RemoteAddress -match $pattern) { $true; break }
        }
        $false
    }
        ...
        foreach ($game in $Script:GameProcessNames) {
            ...
            if ($matches) {
                $gameTimelines += [PSCustomObject]@{
                    Game    = $game
                    Launches = $matches.Count
                    Events   = $matches | Sort-Object Time
                }
            }
        }
